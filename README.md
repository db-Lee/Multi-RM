...
#
# The adapter is merged with its base model automatically inside the
# downloaded/local checkpoint directory (as a `tmp` subdirectory) before
# inference, and that `tmp` directory is deleted again once inference completes.
